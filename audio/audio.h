...
//cx 2018-5-1
bool makeSayMessage_CX(const int &uNum, const double &currentServerTime, const double &ballLastSeenServerTime, const double &ballX, const double &ballY, const double &myX, const double &myY, const bool &fFallen, const bool &turnEasy, const int &chooseCF,const int &chooseFF,const int &chooseCF_noBall,const int &chooseStopper,const int &chooseWL,const int &chooseWR, std::string &message);

bool dataToBits_CX(const double &time, const double &ballLastSeenTime, const double &ballX, const double &ballY, const double &myX, const double &myY, const bool &fFallen, const bool &turnEasy,const int &chooseCF,const int &chooseFF,const int &chooseCF_noBall,const int &chooseStopper,const int &chooseWL,const int &chooseWR, std::vector<int> &bits);
...
bool processHearMessage_CX(const std::string &message, const double &heardServerTime, int &uNum, double &ballLastSeenServerTime, double &ballX, double &ballY, double &agentX, double &agentY, bool &fFallen, bool &turnEasy,int &chooseCF,int &chooseFF,int &chooseCF_noBall,int &chooseStopper,int &chooseWL,int &chooseWR, double &time);

bool bitsToData_CX(const std::vector<int> &bits, double &time, double &ballLastSeenTime, double &ballX, double &ballY, double &agentX, double &agentY, bool &fFallen, bool &turnEasy, int &chooseCF,int &chooseFF,int &chooseCF_noBall,int &chooseStopper,int &chooseWL,int &chooseWR);
